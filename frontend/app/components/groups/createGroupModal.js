...
export default function CreateGroupModal({ onClose, onSubmit }) {  
  ...
  const { fetchToProxyApiCreateGroup, isCreating, fetchGroups } = useGroup({
    ...
    console.log('Refreshing groups data after successful creation');
      ...
      fetchGroups({ page: 1, pageSize: 10, forceRefresh: true }).then(data => {
      ...
      if (onSubmit) {
        const mappedGroup = {
          ...
          coverPhoto: data.image || data.coverPhoto || formData.coverPhoto,
          privacy: data.visibility?.toLowerCase() === "public" ? "public" : "private",
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3 flex-1">
                    <p className="text-sm font-medium text-gray-700 truncate max-w-[180px]">
                      {formData.coverPhotoFile?.name || "Image selected"}
                    </p>
                    <p className="text-xs text-gray-500">
                      {formData.coverPhotoFile ? `${(formData.coverPhotoFile.size / (1024 * 1024)).toFixed(2)} MB` : ''}
                    </p>
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, coverPhoto: "", coverPhotoFile: null }))}
