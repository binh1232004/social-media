...
export default function LeftSidebar() {
    const [isOpen, setIsOpen] = useState(false);    const menuItems = [
        { name: "Home", icon: "🏠", link: "/" },
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className={`w-64 fixed left-0 top-16 h-full p-4 overflow-y-auto bg-white transition-all duration-300 ease-in-out z-40 ${
                isOpen ? 'translate-x-0' : '-translate-x-full'
            } md:translate-x-0 md:hidden lg:block`}>
                <nav className="space-y-2">
                    {menuItems.map((item, index) => (
                        <Link href={item.link} key={index} onClick={() => setIsOpen(false)}>
                            <div className="flex items-center p-3 text-gray-700 hover:bg-gray-200 rounded-lg transition-colors">
                    ...
                    <div className="space-y-2">
                        <Link href="/gaming" onClick={() => setIsOpen(false)}>
                            <div className="flex items-center p-2 hover:bg-gray-200 rounded-lg">
                                <span className="w-8 h-8 bg-blue-500 rounded flex items-center justify-center text-white mr-2">🎮</span>
                                <span>Gaming Group</span>
                            </div>
                        </Link>
                        <Link href="/tech" onClick={() => setIsOpen(false)}>
                            <div className="flex items-center p-2 hover:bg-gray-200 rounded-lg">
                                <span className="w-8 h-8 bg-green-500 rounded flex items-center justify-center text-white mr-2">💻</span>
