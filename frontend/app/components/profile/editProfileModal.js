...
export default function EditProfileModal({ profileData, isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (formData.image && typeof formData.image === 'string' && formData.image !== profileData.avatar) {
      ...
      if (err.response?.status === 413) {
        setError('The image file is too large. Please choose a smaller image.');
      } else if (err.response?.status === 415) {
        setError('The image format is not supported. Please use JPEG, PNG, or GIF.');
